...
impl SearchEngine {
    ...
    #[inline]
    fn search_mov(&mut self, state: &mut State, on_pv: bool, pv_table: &mut [u32], mov: u32, is_capture: bool, alpha: i32, beta: i32, mut depth: u8, mut depth_reduced: bool, mut depth_extend_count: u8, ply: u8, player_sign: i32, node_count: &mut u64, seldepth: &mut u8) -> SearchMovResult {
        if self.abort {
            return Beta(0)
        }
        ...
        if promo != 0 && !def::is_q(promo) && depth >= MIN_REDUCTION_DEPTH {
            depth -= 1;
            depth_extend_count -= 1;
            depth_reduced = true;
        }
...
mod tests {
    ...
    fn test_q_search_4() {
        ...
        assert_eq!(-980, search_engine.q_search(&mut state, -20000, 20000, 0, &mut 0));
    }
