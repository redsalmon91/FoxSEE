...
static WP_PREF_MASK: u64 = 0b00000000_11111111_01111110_01111110_00000000_00000000_00000000_00000000;
static BP_PREF_MASK: u64 = 0b00000000_00000000_00000000_00000000_01111110_01111110_11111111_00000000;
...
pub fn eval_state(state: &State) -> i32 {
    ...
    if (w_piece_count < 2 || b_piece_count < 2) || ((w_piece_count <= 3 && w_pawn_count < 5) || (b_piece_count <= 3 && b_pawn_count < 5)) {
        eval_endgame(state, w_pawn_count, b_pawn_count)
    } else {
        eval_midgame(state)
    }
}
        ...
        match moving_piece {
            def::WP => {
                ...
                if rank > 3 && wp_forward_masks[index] & bitboard.b_pawn == 0 {
                    midgame_score += PASS_PAWN_VAL * rank as i32;
                    midgame_score += PASS_PAWN_VAL * (nearby_masks[index] & w_pawn_mask).count_ones() as i32;
            ...
            def::BP => {
                ...
                if rank > 3 && bp_forward_masks[index] & bitboard.w_pawn == 0 {
                    midgame_score -= PASS_PAWN_VAL * rank as i32;
                    midgame_score -= PASS_PAWN_VAL * (nearby_masks[index] & b_pawn_mask).count_ones() as i32;
...
pub fn eval_endgame(state: &State, w_pawn_count: u32, b_pawn_count: u32) -> i32 {
    let squares = state.squares;
    let index_masks = state.bitmask.index_masks;
    let file_masks = state.bitmask.file_masks;
    let wk_protect_masks = state.bitmask.wk_protect_masks;
        ...
        match moving_piece {
            def::WP => {
                ...
                if pawn_blocker_mask == 0 {
                    endgame_score += PASS_PAWN_VAL * rank as i32;
                    endgame_score += PASS_PAWN_VAL * (nearby_masks[index] & w_pawn_mask).count_ones() as i32;
                } else {
                    ...
                    if nearby_masks[index] & w_pawn_mask == 0 {
                        endgame_score -= ISOLATE_PAWN_PEN;
            ...
            def::BP => {
                ...
                if pawn_blocker_mask == 0 {
                    endgame_score -= PASS_PAWN_VAL * rank as i32;
                    endgame_score -= PASS_PAWN_VAL * (nearby_masks[index] & b_pawn_mask).count_ones() as i32;
                } else {
                    if pawn_blocker_mask.count_ones() == 1 {
                        endgame_score -= PASS_PAWN_VAL;
                    }
...
mod tests {
    ...
    fn test_eval_1() {
        ...
        assert_eq!(250, eval_state(&state));
    }
    ...
    fn test_eval_2() {
        ...
        assert_eq!(1260, eval_state(&state));
    }
    ...
    fn test_eval_7() {
        ...
        let state = State::new("8/6k1/6b1/p5p1/P4pP1/1P1p3P/4p3/2R3K1 b - - 1 39", &zob_keys, &bitmask);
    ...
    fn test_eval_8() {
        ...
        let state = State::new("8/6k1/6b1/p3p1p1/P3p1P1/3p4/8/2R3K1 b - - 1 39", &zob_keys, &bitmask);
        assert_eq!(-405, eval_state(&state));
    }
    ...
    fn test_eval_10() {
        ...
        assert_eq!(155, eval_state(&state));
    }
    ...
    fn test_eval_16() {
        ...
        let state = State::new("r1bq1rk1/3ppppp/P1n5/1P6/8/2N5/5PPP/R1BQ1RK1 w - - 0 1", &zob_keys, &bitmask);
    ...
    fn test_eval_17() {
        ...
        let state = State::new("r1bq1rk1/5ppp/2n5/8/1p6/p1N5/3PPPPP/R1BQ1RK1 w - - 0 1", &zob_keys, &bitmask);
        assert_eq!(-230, eval_state(&state));
    }
    ...
    fn test_eval_18() {
        ...
        let state = State::new("r1b2b1N/3kn1pp/p1p5/1p1pq3/8/1B6/PPP2PPP/R2Q1RK1 w - - 0 1", &zob_keys, &bitmask);
        assert_eq!(255, eval_state(&state));
    }
}
